...
export function BookingSummaryBar({
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex items-center gap-3 border-t border-border bg-card px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-4px_16px_rgb(0_0_0/0.06)]">
      <button
        type="button"
      ...
      >
        <span className="flex w-full items-center gap-1 text-sm font-semibold text-foreground">
        ...
        <span className="text-xs text-muted-foreground">{formatCents(service.price)}</span>
      </button>
      ...
      {step === "confirm" ? (
        ...
        <Button key="confirm" type="submit" form="booking-confirm-form" size="lg" disabled={isSubmitting}>
          {isSubmitting ? "Agendando..." : "Confirmar agendamento"}
        </Button>
      ) : (
        <Button key="continue" type="button" size="lg" onClick={onContinue} disabled={!canContinue}>
          Continuar
        </Button>
      )}
    </div>
